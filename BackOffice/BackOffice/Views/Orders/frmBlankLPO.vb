...
Public Class frmBlankLPO
    Private Function refreshList(supplierId As String, startDate As String, endDate As String)
        dtgrdLPO.Rows.Clear()
        Try
            Dim conn As New MySqlConnection(Database.conString)
            Dim command As New MySqlCommand()
            Dim query As String = ""
            If supplierId <> "" And startDate <> "" And endDate <> "" Then
                query = "SELECT `order_id`, `order_no`, `order_date`, `validity_period`, `valid_until`, `supplier_id`, `status`, `user_id` FROM `orders` WHERE (`order_date`BETWEEN '" + startDate + "' AND '" + endDate + "') AND `supplier_id`='" + supplierId + "' AND `status`='BLANK'"
            ElseIf startDate <> "" And endDate <> "" Then
                query = "SELECT `order_id`, `order_no`, `order_date`, `validity_period`, `valid_until`, `supplier_id`, `status`, `user_id` FROM `orders` WHERE (`order_date`BETWEEN '" + startDate + "' AND '" + endDate + "') AND `status`='BLANK'"
            ElseIf supplierId <> "" Then
                query = "SELECT `order_id`, `order_no`, `order_date`, `validity_period`, `valid_until`, `supplier_id`, `status`, `user_id` FROM `orders` WHERE `supplier_id`='" + supplierId + "' AND `status`='BLANK'"
            Else
                query = "SELECT `order_id`, `order_no`, `order_date`, `validity_period`, `valid_until`, `supplier_id`, `status`, `user_id` FROM `orders` WHERE  `status`='BLANK'"
            End If
            conn.Open()
        ...
        Return vbNull
    End Function
    Private Sub frmLPO_Load(sender As Object, e As EventArgs) Handles MyBase.Shown
        refreshList((New Supplier).getSupplierID("", cmbSupplier.Text), dateStart.Text, dateEnd.Text)
    End Sub
