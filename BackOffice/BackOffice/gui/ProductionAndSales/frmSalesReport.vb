...
Public Class frmDetailedDailySalesReport
    ...
    Private Sub createDocument(doc As Document)
        ...
        documentTitle.AddText("Detailed Daily Sales Report")
        ...
        For i As Integer = 0 To dtgrdList.RowCount - 1
            ...
            Dim price As String = LCurrency.displayValue(dtgrdList.Item(4, i).Value.ToString)
            Dim amount As String = LCurrency.displayValue(dtgrdList.Item(5, i).Value.ToString)
    ...
    Private Function refreshList()
        ...
        Try
            ...
            query = "SELECT
                        `packing_list`.`issue_no` AS `issue_no`,
            ...
            If list <> "" And cmbSalesPersons.Text <> "" Then
                ...
                query = "SELECT
                        ...
                        `packing_list`.`issue_date` AS `issue_date`, 
                        `packing_list`.`status` AS `status`,
                        `packing_list`.`sales_person_id` AS `sales_person_id`,
                        `packing_list_details`.`item_code` AS `item_code`,
                        `packing_list_details`.`description` AS `description`,
                        `packing_list_details`.`price` AS `price`,
                        `packing_list_details`.`cprice` AS `cprice`,
            ...
            While reader.Read
                ...
                dtgrdCell = New DataGridViewTextBoxCell()
                dtgrdCell.Value = "Issue No: " + issueNo
                dtgrdRow.Cells.Add(dtgrdCell)
    ...
    Private Sub btnPrint_Click(sender As Object, e As EventArgs) Handles btnExportToPDF.Click
        ...
        document.Info.Title = "Detailed Daily Sales Report"
        document.Info.Subject = "Detailed Daily Sales Report"
        ...
        Dim filename As String = LSystem.getRoot & "\Detailed Daily Sales Report " & dateStart.Text & dateEnd.Text & cmbSalesPersons.Text & ".pdf"
    ...
    Private Sub btnExportToExcel_Click(sender As Object, e As EventArgs) Handles btnExportToExcel.Click
        ...
        Dim strFileName As String = LSystem.saveToDesktop & "\Detailed Daily Sales Report " & dateStart.Text & dateEnd.Text & cmbSalesPersons.Text & ".xls"
