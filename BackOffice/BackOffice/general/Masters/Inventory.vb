...
Public Class Inventory
    ...
    Public Function getInventory(itemCode As String) As String
        ...
        Return value
    End Function
    Public Function getMinInventory(itemCode As String) As String
        Dim value As String = ""
        Try
            Dim conn As New MySqlConnection(Database.conString)
            Dim command As New MySqlCommand()
            'create bar code
            Dim codeQuery As String = "SELECT  `min_inventory` FROM `inventorys` WHERE `item_code`='" + itemCode + "'"
            conn.Open()
            command.CommandText = codeQuery
            command.Connection = conn
            command.CommandType = CommandType.Text
            Dim reader As MySqlDataReader = command.ExecuteReader()
            While reader.Read
                value = reader.GetString("min_inventory")
                Exit While
            End While
            conn.Close()
        Catch ex As Exception
        ...
        Return value
    End Function
    Public Function getMaxInventory(itemCode As String) As String
        Dim value As String = ""
        Try
            Dim conn As New MySqlConnection(Database.conString)
            Dim command As New MySqlCommand()
            'create bar code
            Dim codeQuery As String = "SELECT  `max_inventory` FROM `inventorys` WHERE `item_code`='" + itemCode + "'"
            conn.Open()
            command.CommandText = codeQuery
            command.Connection = conn
            command.CommandType = CommandType.Text
            Dim reader As MySqlDataReader = command.ExecuteReader()
            While reader.Read
                value = reader.GetString("max_inventory")
                Exit While
            End While
            conn.Close()
        Catch ex As Exception
        ...
        Return value
    End Function
    Public Function getReorderLevel(itemCode As String) As String
        Dim value As String = ""
        Try
            Dim conn As New MySqlConnection(Database.conString)
            Dim command As New MySqlCommand()
            'create bar code
            Dim codeQuery As String = "SELECT  `reorder_level` FROM `inventorys` WHERE `item_code`='" + itemCode + "'"
            conn.Open()
            command.CommandText = codeQuery
            command.Connection = conn
            command.CommandType = CommandType.Text
            Dim reader As MySqlDataReader = command.ExecuteReader()
            While reader.Read
                value = reader.GetString("reorder_level")
                Exit While
            End While
            conn.Close()
        Catch ex As Exception
